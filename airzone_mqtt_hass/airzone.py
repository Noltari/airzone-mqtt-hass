...
class Airzone:
    ...
    async def ha_config_pub(self) -> None:
        ...
        for system in self.systems.values():
            system_config = system.ha_mqtt_config()
            _LOGGER.error("ha_config_pub: system_config=%s", system_config)

        for zone in self.zones.values():
            zone_config = zone.ha_mqtt_config()
            _LOGGER.error("ha_config_pub: zone_config=%s", zone_config)
